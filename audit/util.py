...
class ProcessAuditData:
    ...
    def process_log_details(self, obj):
        try:
            ...
            directory = os.path.dirname(self.full_file_name)
    ...
    def trigger_process(self, obj):
        try:
            ...
            self.process_log.files_processed = self.process_log.files_processed + 1
    ...
    def generate_output_report(self):
        ...
        output_df = self.generate_compared_report(df, self.writer, "output")
        temp_df = output_df.copy()
        self.generate_bin_reports(bin_df, self.writer, temp_df)
        ...
        self.process_log.save()
        if os.path.exists(self.full_file_name) and os.path.isfile(self.full_file_name):
            os.remove(self.full_file_name)
        folder_to_remove = os.path.dirname(self.full_file_name)
        if os.path.exists(folder_to_remove) and os.path.isdir(folder_to_remove):
            os.rmdir(folder_to_remove)  # This removes the folder and all its contents

    def generate_bin_reports(self, bin_df, writer, temp_df):
        bin_groups = bin_df["bgp_name"].unique()
        bin_df["NDC"] = bin_df["NDC"].astype(str).apply(lambda x: self.format_NDC(x))
        bin_df = bin_df.fillna(0)
        columns_to_keep = ["NDC", "Drug Name", "description", "distributor_quantity"]
        temp_df.loc[:, ~temp_df.columns.isin(columns_to_keep)] = temp_df.loc[
            :, ~temp_df.columns.isin(columns_to_keep)
        ].applymap(lambda x: 0 if isinstance(x, (int, float)) else "0")
        for bin_group in sorted(bin_groups):
            df = bin_df[bin_df["bgp_name"] == bin_group]
            df = df.drop(["bgp_name"], axis=1)
            temp_df_filtered = temp_df[~temp_df["NDC"].isin(df["NDC"])]
            df = pd.concat([temp_df_filtered, df], ignore_index=True)
            df = df.fillna(0)
...
def handle_zip_file(file, process_log, obj, pharmacy):
    ...
    extract_to_folder = os.path.join(temp_dir, f"extracted_{process_log.name}")
    ...
    try:
        ...
        process_log.total_files = len(extracted_files) + 3
        process_log.save()
        ...
        for extracted_file in extracted_files:
            ...
            output_folder = os.path.join(
                os.getcwd(),
                settings.AUDIT_FILES_LOCATION,
                process_log.name,  # Use process_log.name as the folder name
            )

            # Ensure the folder exists before saving files
            os.makedirs(output_folder, exist_ok=True)

            # Save the cleaned file inside the process-specific folder
            output_file = os.path.join(
                output_folder, f"cleaned_{os.path.basename(base_name)}"
            )
            created_files.append(output_file)
            try:
                clean_file_and_retreive_output_file(base_name, output_file)
                process_log.files_processed = process_log.files_processed + 1
                process_log.save()
            except Exception as e:
        ...
        # ):
        process_log.save()
        process_audit_data.trigger_process(process_log)
        ...
        if os.path.exists(output_folder):
            ...
            os.rmdir(output_folder)  # Finally, remove the empty folder
