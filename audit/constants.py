...
def get_output_report_sql(
    ...
    FIRST_VALUE(COALESCE(pharmacy_data."Drug Name", distributor_data.dad_drug_name))
        OVER (PARTITION BY COALESCE(pharmacy_data."NDC", distributor_data.dad_ndc)
              ORDER BY CASE WHEN pharmacy_data."Drug Name" IS NOT NULL THEN 0 ELSE 1 END) AS "Drug Name",
    ...
    distributor_data.distributor_quantity    
FROM (SELECT              
        pad_ndc AS "NDC",            
        pad_brand AS "Brand",            
        ...
        (SUM(pad_quantity) / COALESCE(NULLIF(CAST(pad_size AS DECIMAL), 0), 1)) AS "Dispense Qty in Packs",            
     ...
     WHERE pad_process_log_id = '{process_log_id}'
         {pharmacy_date_filter}
         {payment_option_filter}
         {claim_status_filter}
         {group_filter}
         {pcn_filter}
         {bin_filter}      
     GROUP BY pad_ndc, pad_brand, pad_strength, pad_drug_name, pad_size        
    ) AS pharmacy_data    
FULL OUTER JOIN (
    SELECT              
        dad_ndc,              
    ...
    FROM "OPT_DAD_DistributorAuditData"        
    JOIN "OPT_DTB_Distributors"          
    ON "OPT_DTB_Distributors".id = dad_distributor        
    WHERE dad_process_log_id = '{process_log_id}'
         {distributor_date_filter}        
    GROUP BY dad_ndc, description, dad_drug_name        
) AS distributor_data    
ON pharmacy_data."NDC" = distributor_data.dad_ndc
GROUP BY          
    COALESCE(pharmacy_data."NDC", distributor_data.dad_ndc),
    distributor_data.distributor_quantity,
    pharmacy_data."Drug Name",
    distributor_data.dad_drug_name,description
